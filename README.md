...
Rclone is A meltano utility extension for Rclone that wraps the `rclone` command.
...
```shell
poetry run rclone_extension --help
poetry run rclone_extension describe --format=yaml
poetry run rclone_invoker --help # if you have are wrapping another tool
```
